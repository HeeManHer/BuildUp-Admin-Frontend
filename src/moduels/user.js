...
const userReducer = handleActions(
    {
        ...
        [SEARCH_USER]: (state, { payload }) => {
            const result1 = state.filter(user => user.no === payload);
            const result2 = state.filter(user => user.name === payload);
            const result3 = state.filter(user => user.email === payload);
            const result4 = state.filter(user => user.authority === payload);
